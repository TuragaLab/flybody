...
def terrain_bowl(physics, bump_scale=2., elevation_z=4., tanh_rel_radius=0.7,
                 tanh_sharpness=8., random_state=None):
  ...
  r = np.sqrt(xv**2 + yv**2)
  bowl_shape = 0.5 * np.tanh(
    tanh_sharpness * (r - tanh_rel_radius)) + 0.5  # Between 0 and 1.
  terrain *= bowl_shape
...
def add_sine_trench(terrain, arena_size, wavelength=5, phase=0., amplitude=1.,
                    start_x=0, end_x=10., width=1., height=1., sigma=0.2):
  ...
  delta, _ = pos_to_terrain_idx(-arena_size[0]+width/2, 0, arena_size, nrow,
                                ncol)
  ...
  for idx_x in range(idx_from, idx_to):
    _, idx_y = pos_to_terrain_idx(0, sine[idx_x-idx_from], arena_size, nrow,
                                  ncol)
...
class Hills(composer.Arena):
  ...
  """
  def _build(self, name='hills', dim=20, aesthetic='outdoor_natural',
             hfield_elevation_z=1, hfield_base_z=0.05, grid_density=10,
  ...
  def initialize_episode(self, physics, random_state):
    if self._regenerate:
      ...
      terrain = terrain_bowl(physics, elevation_z=elevation_z,
...
class SineTrench(Hills):
  ...
  def _build(self, name='sine_trench', dim=20, aesthetic='outdoor_natural',
             hfield_elevation_z=1, hfield_base_z=0.05, grid_density=10,
             elevation_z_range=(4., 5.), start_offset_range=(-5., -3.),
             trench_len_range=(4., 10.), phase_range=(0., 2*np.pi),
             wavelength_range=(5., 8.), amplitude_range=(0.7/2, 1.2/2),
             width_range=(0.5, 1), height_range=(1.3, 1.3),
             sigma_range=(0.2, 0.2)):

    super()._build(dim=dim, aesthetic=aesthetic, name=name,
                   hfield_elevation_z=hfield_elevation_z,
                   hfield_base_z=hfield_base_z, grid_density=grid_density,
  ...
  def initialize_episode(self, physics, random_state):
    if self._regenerate:
      ...
      # Create bowl arena.
      elevation_z = random_state.uniform(*self._elevation_z_range)
      bowl = terrain_bowl(physics, elevation_z=elevation_z,
                          random_state=random_state)
      ...
      terrain, sine = add_sine_trench(bowl, size, start_x=start_x,
                                      end_x=end_x,
                                      phase=random_state.uniform(
                                        *self._phase_range),
                                      wavelength=random_state.uniform(
                                        *self._wavelength_range),
                                      amplitude=amplitude,
                                      width=width,
                                      height=random_state.uniform(
                                        *self._height_range),
                                      sigma=random_state.uniform(
                                        *self._sigma_range))
...
class SineBumps(Hills):
  ...
  def _build(self, name='sine_bumps', dim=20, aesthetic='outdoor_natural',
             hfield_elevation_z=1, hfield_base_z=0.05, grid_density=10,
             elevation_z_range=(4., 5.),
             phase_range=(0., 2*np.pi), wavelength_range=(10., 15.),
  ...
  def initialize_episode(self, physics, random_state):
    if self._regenerate:
      ...
      # Add sine trench.
      terrain = add_sine_bumps(bowl, size,
                               wavelength=random_state.uniform(
                                 *self._wavelength_range),
