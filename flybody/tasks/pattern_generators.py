...
class WingBeatPatternGenerator():
  ...
  def __init__(self,
    ...
    for beat_freq in self.beat_freqs:
      ...
      phase = np.linspace(0, n_reps, n_reps * base_pattern.shape[0],
                          endpoint=False)
  ...
  def reset(self,
    ...
    self._step = np.argmin(np.abs(
      initial_phase - self.traj_ctrl[self._freq_idx]['phase']))

    if return_qvel:
      return (self._traj[self._step, :], (self._traj[self._step+1, :]
              - self._traj[self._step, :]) / self._dt_ctrl)
  ...
  def step(self, ctrl_freq: float) -> np.ndarray:
    ...
    if self.ctrl_filter == 0.:
      ...
      self._ctrl_freq = (self._ctrl_freq * self._rate
                         + ctrl_freq * (1 - self._rate))
    ...
    if idx_new != self._freq_idx:
      ...
      # within beat cycle.
      self._step = np.argmin(np.abs(
        current_phase % 1 - self.traj_ctrl[idx_new]['phase'] % 1))
