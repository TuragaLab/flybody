...
_XML_PATH = os.path.join(os.path.dirname(__file__), 'assets/fruitfly.xml')
# === Constants.
...
class FruitFly(legacy_base.Walker):
    ...
    def _build(
        ...
        if not use_wings:
            ...
            for joint in wing_joints:
                ...
                self.observable_joints.remove(joint)
            # Remove wing sensors, if there are any.
            for sensor in root.find_all('sensor'):
        ...
        for act_class in self._num_actions.keys():
            if self._ctrl_indices[act_class] is not None:
                self._num_actions[act_class] = len(
                    self._ctrl_indices[act_class])
        ...
        for act_class in _ACTION_CLASSES.keys():
            if self._num_actions[act_class]:
                indices = list(
                    range(counter, counter + self._num_actions[act_class]))
    ...
    def mocap_tracking_bodies(self):
        ...
        body_names = ('thorax', 'abdomen', 'head', 'claw_T1_left',
                      'claw_T1_right', 'claw_T2_left', 'claw_T2_right',
    ...
    def end_effectors(self):
        site_names = ('claw_T1_left', 'claw_T1_right', 'claw_T2_left',
    ...
    def ground_contact_geoms(self):
        ...
        return (
            self._mjcf_root.find('geom', 'tarsal_claw_T1_left_collision'),
    ...
    def get_action_spec(self, physics):
        ...
        names = [
            physics.model.id2name(i, 'actuator') or str(i) for i in indices
        ]
        ...
        if self._num_actions['user']:
            ...
            names.extend(
                [f'user_{i}' for i in range(self._num_actions['user'])])
...
class FruitFlyObservables(legacy_base.WalkerObservables):
    ...
    def appendages_pos(self):
        ...
        def relative_pos_in_egocentric_frame(physics):
            ...
            torso_mat = np.reshape(
                physics.bind(self._entity.root_body).xmat, (3, 3))
    ...
    def proprioception(self):
        ...
        return [self.joints_pos, self.joints_vel, self.actuator_activation]

    @property
    def orientation(self):
        """Return orientation of world z-axis in local frame."""
        return [
            self.world_zaxis, self.world_zaxis_abdomen, self.world_zaxis_head
        ]
