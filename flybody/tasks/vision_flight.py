...
from flybody.tasks.pattern_generators import (
...
class VisionFlightImitationWBPG(Flying):
  ...
  def __init__(
    ...
    **kwargs
    ...
    super().__init__(add_ghost=False, num_user_actions=1,
  ...
  def get_hfield_height(self, x, y, physics):
    ...
    self._grid_axis = np.linspace(
        -hfield_half_size, hfield_half_size, self._ncol)
  ...
  def initialize_episode(self, physics: 'mjcf.Physics',
    ...
    self._walker.set_pose(physics,
                          np.array([init_x, init_y, init_z]),
    ...
    if self._initialize_qvel:
        ...
        self._walker.set_velocity(physics,
                                  [self._target_speed, init_vel[1],
                                   init_vel[2]])
  ...
  def check_termination(self, physics: 'mjcf.Physics') -> bool:
    if self._floor_contacts_fatal:
      return (self.check_floor_contact(physics) or
  ...
  def task_input(self):
    ...
    def get_task_input(physics: 'mjcf.Physics'):
      del physics
      return np.hstack([self._target_height,
                        self._target_speed])
