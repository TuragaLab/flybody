...
def main(argv: Sequence[str]):
    ...
    sort_order = [
        'free', 'thorax', 'head', 'wing', 'abdomen', 'haltere_left',
        'haltere_right', 'coxa'
    ]
    ...
    rostrum.find('joint', 'rx_rostrum').springref = 0.8
    ...
    links = [
        'coxa', 'femur', 'tibia', 'tarsus', 'tarsus2', 'tarsus3', 'tarsus4'
    ]
    sternums = ['T1', 'T2', 'T3']
    sides = ['right', 'left']
    ...
    tarsi4 = [
        body for body in thorax.find_all('body') if 'tarsus4' in body.name
    ]
    for tarsus4 in tarsi4:
        gclaw = [
            geom for geom in tarsus4.get_children('geom')
            if 'claw' in geom.name
        ][0]
        ipos = tarsus4.pos
        claw = tarsus4.add('body',
                           name=tarsus4.name.replace('tarsus4', 'claw'),
                           pos=ipos)
        ...
        for geom in leg_meshes:
            ...
            if len(children) == 1:
                ...
                if 'coxa' in geom.name:
                    fromto = None
                    if 'T3' in geom.name:
                        axis = children[0].pos / np.linalg.norm(
                            children[0].pos)
                    ...
                    if 'femur' in geom.name:
                        ...
                        if 'T3' in geom.name:
                            fromto[0] += 0.002625 * (-1 if 'left' in geom.name
                                                     else 1)
                            fromto[3] += 0.002625 * (-1 if 'left' in geom.name
                                                     else 1)
                            fromto[4] += 0.002625 * (1 if 'left' in geom.name
                                                     else -1)
                        if 'T1' in geom.name:
                            fromto[0] += .002625 * (1 if 'left' in geom.name
                                                    else -1)
                        size = (1.05 * size[0], )
    ...
    for geom in wing_meshes:
        ...
        index = geom.parent._children.index(geom)  # pylint: disable=protected-access
        colgeom = geom.parent.insert('geom',
                                     index + 1,
                                     pos=pos,
                                     quat=quat,
                                     size=size,
    ...
    thorax.insert('geom',
                  2,
                  name='thorax_collision',
                  dclass='collision',
                  type='ellipsoid',
                  size=(0.04375, 0.04375, 0.055125),
                  pos=(-0.0175, 0, -0.002625),
                  quat=rotate)
    angle = -.4
    rotate = np.array((np.cos(angle / 2), 0, np.sin(angle / 2), 0))
    thorax.insert('geom',
                  3,
                  name='thorax_collision2',
                  dclass='collision',
                  type='ellipsoid',
                  size=(0.049875, 0.028, 0.011375),
                  pos=(-0.011375, 0, 0.021875),
                  quat=rotate)
    # Head.
    ...
    head.insert('geom',
                ...
                dclass='collision',
                type='ellipsoid',
                size=(0.0455, 0.02625, 0.032375),
                pos=(0, 0.014875, 0.000875),
                euler=(.3, 0, 0))
    # Eye cameras.
    mat = np.zeros((3, 3))
    mat[:, 0] = [.45, -1, -.3]
    ...
    rostrum.insert('geom',
                   2,
                   name='rostrum_collision',
                   dclass='collision',
                   type='ellipsoid',
                   size=(0.013125, 0.021875, 0.013125),
                   pos=(0, 0.0175, 0.002625),
                   euler=(.1, 0, 0))
    for side in ['_left', '_right']:
        fromto = np.array(
            (-0.006125, 0.032375, 0, -0.011375, 0.0245, -0.023625))
        if 'r' in side:
            fromto[0] *= -1
            fromto[3] *= -1
        rostrum.insert('geom',
                       2,
                       name='rostrum_collision' + side,
                       dclass='collision',
                       size=(0.0035, ),
                       fromto=fromto)
    ...
    for body in model.find_all('body'):
        ...
        if sum(joint_index >= 0) > 1:
            joint_reorder = [
                joint_index[i] for i in [2, 1, 0] if joint_index[i] >= 0
            ]
    ...
    for haltere in halteres:
        if 'left' in haltere.name:
            ...
            change_body_frame(haltere, haltere.pos,
                              mul_quat(haltere.quat, rotate_y))
    ...
    for leg in legs:
        ...
        while parent:
            ...
            for joint in parent.get_children('joint'):
                ...
                joint.range = None
                joint.axis = None
                def_name = joint.name.replace('_left',
                                              '').replace('_right', '')
                if not model.find('default', def_name):
                    ...
                    if not model.find('default', def_name):
                        ...
                        if not model.find('default', def_name):
                            raise ValueError(
                                'Default class not found for joint.')
    ...
    for joint in model.find_all('joint'):
        ...
        if 'abdomen' in joint.name:
            for tendon_name in abdomen_tendons:
                if joint.name == tendon_name:
                    ...
                    num_joints = len(
                        abdomen_tendons[tendon_name].get_children('joint'))
                    model.actuator.add('general',
                                       name=tendon_name,
                                       tendon=abdomen_tendons[tendon_name],
                                       dclass=dclass,
                                       ctrlrange=num_joints *
                                       dclass.joint.range)
            continue
        ...
        if ('twist' in joint.name or 'abduct' in joint.name
            ...
            if ('coxa' in joint.name or 'femur' in joint.name
                    or 'tibia' in joint.name or 'tarsus' in joint.name):
                assert (dclass.joint.range is not None
    ...
    for body in model.find_all('body'):
        if 'claw' in body.name:
            model.actuator.add('adhesion',
                               name='adhere_' + body.name,
                               body=body.name,
                               dclass='adhesion_claw')
        if 'labrum' in body.name:
            model.actuator.add('adhesion',
                               name='adhere_' + body.name,
                               body=body.name,
                               dclass='adhesion_labrum')
    ...
    for con in physics.data.contact:
        body1 = physics.model.id2name(physics.model.geom_bodyid[con.geom1],
                                      'body')
        body2 = physics.model.id2name(physics.model.geom_bodyid[con.geom2],
                                      'body')
    ...
    thorax.insert('site',
                  2,
                  name='hover_up_dir',
                  quat=np.array([np.cos(angle / 2), 0,
                                 np.sin(angle / 2), 0]),
    ...
    touch_sites = []
    force_sites = []
    inflate_claw_site = 1.1  # Inflate claw site for touch sensor active zone.
    for leg in legs:
                ...
                for geom in body.get_children('geom'):
                    if 'collision' in geom.name:
                        site = body.add('site',
                                        name=body.name,
                                        dclass='adhesion-collision',
                                        fromto=geom.fromto * inflate_claw_site,
                                        size=geom.size * inflate_claw_site,
                                        )
