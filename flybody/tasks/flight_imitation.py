...
class FlightImitationWBPG(Flying):
  ...
  def __init__(
    ...
    **kwargs
    ...
    for i, fromto in enumerate([[0, 0, 0, 0, 0, 1],
                                [-0.5, 0, 0.5, 0.5, 0, 0.5],
                                [0, -0.5, 0.5, 0, 0.5, 0.5]]):
      self._crosshair_sites.append(world.add('site', name=f'crosshair_{i}',
                                             type='capsule', size=(0.002,),
                                             fromto=fromto, 
                                             rgba=(0, 0.8, 0.5, 0.5)))
    ...
    if self._trajectory_sites:
      self._n_traj_sites = (round(self._time_limit / self.control_timestep)
                            + 1) // 10
  ...
  def before_step(self, physics: 'mjcf.Physics', action,
    ...
    ghost_qpos = self._ref_qpos[step, :] + np.hstack((self._ghost_offset,
                                                      4*[0]))
    self._ghost.set_pose(physics, ghost_qpos[:3], ghost_qpos[3:])
  ...
  def check_termination(self, physics: 'mjcf.Physics') -> bool:
    ...
    return (height < _TERMINAL_HEIGHT
